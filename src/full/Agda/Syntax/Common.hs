...
data UserOrigin
  = UserOrigin        -- ^ From source file or from interaction.
  | FromEllipsis      -- ^ Expanded from ellipsis @...@.
  | ModifiedEllipsis  -- ^ Originally from ellipsis, but then modified by case split.
  deriving (Typeable, Data, Show, Eq, Ord)

instance NFData UserOrigin where
  rnf UserOrigin = ()
  rnf FromEllipsis = ()
  rnf ModifiedEllipsis = ()

-- | Mark things coming from ellipsis as modified.
touchUserOrigin :: UserOrigin -> UserOrigin
touchUserOrigin FromEllipsis = ModifiedEllipsis
touchUserOrigin u = u

data Origin
  = UserWritten UserOrigin
  | Inserted     -- ^ Inserted by the system
  | Reflected    -- ^ Created from reflected syntax
  deriving (Typeable, Data, Show, Eq, Ord)

-- | The default variant of 'UserWritten'.
userWritten :: Origin
userWritten = UserWritten UserOrigin

isUserWritten :: LensOrigin a => a -> Bool
isUserWritten a = case getOrigin a of
  UserWritten{} -> True
  _ -> False

-- | Mark something as written by user.
--   Remember whether it came originally from ellipsis.
mkUserWritten :: Origin -> Origin
mkUserWritten (UserWritten u) = UserWritten $ touchUserOrigin u
mkUserWritten _ = userWritten

instance KillRange Origin where
  killRange = id

instance NFData Origin where
  rnf (UserWritten uo) = rnf uo
  rnf Inserted = ()
  rnf Reflected = ()
...
defaultArgInfo =  ArgInfo { argInfoHiding       = NotHidden
                          ...
                          , argInfoOrigin       = UserWritten UserOrigin
