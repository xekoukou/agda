...
data MetaInstantiation
        = InstV ListTel Term -- ^ solved by term (abstracted over some free variables)
        | Open               -- ^ unsolved
...
instance Show MetaInstantiation where
  show (InstV tel t) = "InstV " ++ show (telToArgs tel) ++ " (" ++ show t ++ ")"
  show Open      = "Open"
...
projDropPars (Projection Just{} d _ _ lams) o =
  case initLast $ getProjLams lams of
    Nothing -> Def d []
    Just (pars, Arg i y) ->
      let core = untypedLam i $ Abs y $ Var 0 [Proj o d] in
      List.foldr (\ (Arg ai x) -> untypedLam ai . NoAbs x) core pars
...
projDropPars (Projection Nothing d _ _ lams) o =
  List.foldr (\ (Arg ai x) -> untypedLam ai . NoAbs x) (Def d []) $ init $ getProjLams lams
